...
import { Session as SessionType, SteamUser } from './types';
...
export class Session {
    ...
    private startHeartbeat(): boolean {
        ...
        this.heartbeatTask = new AsyncTask(
            ...
            (): Promise<void> => {
                ...
                return <Promise<void>><any>deferred.promise;
            },
            (e) => {
                logger.error(e);
            }
    ...
    getSession(tryNum = 0, username = 'oldest'): Promise<SessionType> {
        logger.silly('getSession');
        const deferred = Q.defer();
        if (tryNum > 3) {
            return new Promise((_resolve, reject) => reject({ error: 'max retries' }));
        }
        if (this.needsNewSession()) {
            logger.silly('new session needed');
            var promise: Q.Promise<SteamUser> | undefined = undefined;
            if (username === 'oldest') {
                promise = SteamUserFunctions.getOldest(this.type);
            } else {
                promise = SteamUserFunctions.getUser(username);
            }
            promise.then((steamUser: SteamUser) => {
                if (steamUser === null)
                        ...
                        publicPost(getPath(endpoint), postData).then((result) => {
                            SteamUserFunctions.addKardsLogin(steamUser.username).then(() => {
                                if (_.isObject(result)) {
                                    this.session = <SessionType>result;
                                    this.startHeartbeat();
            ...
            return new Promise<SessionType>((resolve, _reject) => {
                if (this.session === undefined)
                    return reject(new Error('Session invalidated before it was returned'));
                return resolve(this.session)
            });
        }
        return <Promise<SessionType>><any>deferred.promise;
    }
    ...
    refreshSteam(username: string): Promise<string> {
        logger.silly('refreshSteam');
        const deferred = Q.defer();

        SteamUserFunctions.getUser(username).then((steamUser) => {
